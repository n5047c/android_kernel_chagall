...
struct tegra_usb_modem {
	...
	struct delayed_work autosuspend_work; /* 3g module auto-suspend work*/
...
static const struct usb_device_id modem_list[] = {
	{USB_DEVICE(0x12D1, 0x1506),	/* MU739 */
...
static void tegra_usb_enable_autosuspend(struct work_struct *ws)
{
	struct tegra_usb_modem *modem = container_of(ws, struct tegra_usb_modem,
										autosuspend_work.work);
	if (modem->udev)
		usb_enable_autosuspend(modem->udev);
}
				  ...
				  struct usb_device *udev)
{
	...
	if (desc->idVendor == modem->vid && desc->idProduct == modem->pid) {
		...
		/*schedule recovery work if sim card detected*/
		if (TEGRA_MODEM_AUTOSUSPEND
			&& modem->ops && modem->ops->get_status
			&& !modem->ops->get_status())
			queue_delayed_work(modem->wq,
					   &modem->recovery_work, HZ * 5);
	}
}
				    ...
				    const char *label, unsigned int *irq)
{
	...
	if (ret) {
		printk("%s: gpio_request fail, ret = %d\n", __func__, ret);
		return ret;
	}
...
static int mdm_init(struct tegra_usb_modem *modem, struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&modem->autosuspend_work, tegra_usb_enable_autosuspend);
...
fs_initcall(tegra_usb_modem_power_init);
