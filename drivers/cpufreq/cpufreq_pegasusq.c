...
static void start_rq_work(void)
{
	...
	if (rq_data->nr_run_wq == NULL)
		rq_data->nr_run_wq =
			alloc_workqueue("nr_run_avg", 0, 1);
...
static inline cputime64_t get_cpu_idle_time_my(unsigned int cpu, cputime64_t *wall)
{
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		...
		dbs_info->prev_cpu_idle =
			get_cpu_idle_time_my(j, &dbs_info->prev_cpu_wall);
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = get_cpu_idle_time_my(j, &cur_wall_time);
				...
				unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = get_cpu_idle_time_my(j,
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dvfs_workqueue = alloc_workqueue("kpegasusq", WQ_HIGHPRI, 1);
	if (!dvfs_workqueue) {
