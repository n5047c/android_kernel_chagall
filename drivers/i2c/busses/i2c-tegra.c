...
struct tegra_i2c_dev {
	...
	void (*i2c_bus_reset_devices)(struct tegra_i2c_platform_data *pd,
		u16 client_addr);
	...
	struct i2c_msg *msg, enum msg_end_type end_state)
{
	...
	if (WARN_ON(ret == 0)) {
		...
		if(i2c_dev->i2c_bus_reset_devices) {
			dev_info(i2c_dev->dev, "===============i2c reset device==============\n");
			i2c_dev->i2c_bus_reset_devices(i2c_dev->dev->platform_data, msg->addr);
		}
...
static int __devinit tegra_i2c_probe(struct platform_device *pdev)
{
	...
	i2c_dev->i2c_bus_reset_devices = plat->i2c_bus_reset_devices;
	init_completion(&i2c_dev->msg_complete);
