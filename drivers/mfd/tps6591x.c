...
#define TPS6591X_MAX_I2C_RETRY_COUNTS 20
...
static inline int __tps6591x_read_retry(struct i2c_client *client,
				  int reg, uint8_t *val)
{
	int i, ret;

	for (i=0; i < TPS6591X_MAX_I2C_RETRY_COUNTS; i++) {
		ret = __tps6591x_read(client, reg, val);
		if (ret >= 0) {
			return ret;
		}
...
static inline int __tps6591x_reads_retry(struct i2c_client *client, int reg,
				int len, uint8_t *val)
{
	int i, ret;

	for (i=0; i < TPS6591X_MAX_I2C_RETRY_COUNTS; i++) {
		ret = __tps6591x_reads(client, reg, len, val);
		if (ret >= 0) {
			return ret;
		}
		dev_err(&client->dev, "%s:retry:%d\n", __FUNCTION__, i);
...
static inline int __tps6591x_write_retry(struct i2c_client *client,
				 int reg, uint8_t val)
{
	int i, ret;

	for (i=0; i < TPS6591X_MAX_I2C_RETRY_COUNTS; i++) {
		ret = __tps6591x_write(client, reg, val);
		if (ret >= 0) {
			return ret;
		}
		dev_err(&client->dev, "%s:retry:%d\n", __FUNCTION__, i);
...
static inline int __tps6591x_writes_retry(struct i2c_client *client, int reg,
				  int len, uint8_t *val)
{
	int i, ret;

	for (i=0; i < TPS6591X_MAX_I2C_RETRY_COUNTS; i++) {
		ret = __tps6591x_writes(client, reg, len, val);
		if (ret >= 0) {
			return ret;
		}
		dev_err(&client->dev, "%s:retry:%d\n", __FUNCTION__, i);
...
int tps6591x_write(struct device *dev, int reg, uint8_t val)
{
	...
	ret = __tps6591x_write_retry(to_i2c_client(dev), reg, val);
...
int tps6591x_writes(struct device *dev, int reg, int len, uint8_t *val)
{
	...
	mutex_lock(&tps6591x->lock);
	ret = __tps6591x_writes_retry(to_i2c_client(dev), reg, len, val);
	mutex_unlock(&tps6591x->lock);
...
{
	return __tps6591x_read_retry(to_i2c_client(dev), reg, val);
}
EXPORT_SYMBOL_GPL(tps6591x_read);

int tps6591x_reads(struct device *dev, int reg, int len, uint8_t *val)
{
	return __tps6591x_reads_retry(to_i2c_client(dev), reg, len, val);
}
...
int tps6591x_set_bits(struct device *dev, int reg, uint8_t bit_mask)
{
	...
	ret = __tps6591x_read_retry(to_i2c_client(dev), reg, &reg_val);
	if (ret)
		goto out;

	if ((reg_val & bit_mask) != bit_mask) {
		reg_val |= bit_mask;
		ret = __tps6591x_write_retry(to_i2c_client(dev), reg, reg_val);
	}
out:
	mutex_unlock(&tps6591x->lock);
	return ret;
...
int tps6591x_update(struct device *dev, int reg, uint8_t val, uint8_t mask)
{
	...
	ret = __tps6591x_read_retry(tps6591x->client, reg, &reg_val);
	if (ret)
		goto out;

	if ((reg_val & mask) != val) {
		reg_val = (reg_val & ~mask) | (val & mask);
		ret = __tps6591x_write_retry(tps6591x->client, reg, reg_val);
	}
out:
	mutex_unlock(&tps6591x->lock);
	return ret;
...
static void tps6591x_power_off(void)
{
	...
	tps6591x_set_bits(dev, TPS6591X_DEVCTRL, DEV_OFF);
...
static int tps6591x_gpio_get(struct gpio_chip *gc, unsigned offset)
{
	...
	ret = __tps6591x_read_retry(tps6591x->client, TPS6591X_GPIO_BASE_ADDR +
			offset,	&val);
	if (ret)
		return ret;
...
static int tps6591x_gpio_input(struct gpio_chip *gc, unsigned offset)
{
	...
	reg_val &= ~0x4;
	return __tps6591x_write_retry(tps6591x->client, TPS6591X_GPIO_BASE_ADDR +
			offset,	reg_val);
}
			...
			struct tps6591x_platform_data *pdata)
{
	...
	for (i = 0; i < pdata->num_gpioinit_data; ++i) {
		...
		ret =  __tps6591x_write_retry(tps6591x->client,
		...
		int end_offset)
{
	...
	for (i = start_offset; i <= end_offset; ++i) {
		ret = __tps6591x_read_retry(client, i, &reg_val);
		if (ret >= 0)
