...
struct usb_device *usb2_roothub;

void usb2_autosuspend(int en)
{
	if(usb2_roothub) {
		if(en)
			usb_enable_autosuspend(usb2_roothub);
		else
			usb_disable_autosuspend(usb2_roothub);
	}
}

EXPORT_SYMBOL(usb2_autosuspend);
