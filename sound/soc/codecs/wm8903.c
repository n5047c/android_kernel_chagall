	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->dapm.bias_level == SND_SOC_BIAS_OFF) {
			snd_soc_update_bits(codec, WM8903_WRITE_SEQUENCER_0,
				WM8903_WSEQ_ENA_MASK | WM8903_WSEQ_WRITE_INDEX_MASK,
				WM8903_WSEQ_ENA | (0x11 << WM8903_WSEQ_WRITE_INDEX_SHIFT));
		      ...
		      int det, int shrt)
{
	...
	/* reset mic_last_report when switching mic detect */
	if ((wm8903->mic_det ^ det) || (wm8903->mic_short ^ shrt)) {
		wm8903->mic_last_report = 0;
	}
...
#if 0
	val = snd_soc_read(codec, WM8903_ANALOGUE_LEFT_INPUT_0);
#else
	val = pdata->adc_analogue_volume;
#endif
	snd_soc_write(codec, WM8903_ANALOGUE_LEFT_INPUT_0, val);
	snd_soc_write(codec, WM8903_ANALOGUE_RIGHT_INPUT_0, val);

#if 0
	val = snd_soc_read(codec, WM8903_DAC_DIGITAL_VOLUME_LEFT);
#else
	val = pdata->dac_digital_volume;
#endif
	val |= WM8903_DACVU;
...
#else
	val = pdata->dac_headphone_volume;
#endif
	val |= WM8903_HPOUTVU;
